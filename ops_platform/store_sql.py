"""SQLPlatformStore —— SQLAlchemy 版本的 platform store（生产用）。

完整设计说明见 ops_platform/store/__init__.py 或 store.py。
"""
...
class SQLPlatformStore:
    ...
    def _ensure_columns(self) -> None:
        ...
        with self.engine.begin() as conn:
            ...
            # platform_model_config.tool_choice_preference —— 老 DB 没这列
            try:
                conn.execute(text(
                    "SELECT tool_choice_preference FROM platform_model_config LIMIT 1"
                ))
            except Exception:
                try:
                    conn.execute(text(
                        "ALTER TABLE platform_model_config "
                        "ADD COLUMN tool_choice_preference VARCHAR(16)"
                    ))
                    logger.info("迁移：platform_model_config 加 tool_choice_preference 列完毕")
                except Exception as exc:    # pragma: no cover
                    logger.warning(
                        "加 tool_choice_preference 列失败（如已存在/老 DB 不支持）：%s", exc,
                    )
    ...
    def _sqlite_ddl() -> list[str]:
        return [
            ...
            CREATE TABLE IF NOT EXISTS platform_model_config (
                ...
                is_default INTEGER NOT NULL DEFAULT 0,
                enabled INTEGER NOT NULL DEFAULT 1,
                tool_choice_preference VARCHAR(16),
                created_by VARCHAR(128),
                created_at VARCHAR(64),
                updated_at VARCHAR(64)
            )
    ...
    def _mysql_ddl() -> list[str]:
        return [
            ...
            CREATE TABLE IF NOT EXISTS platform_model_config (
                ...
                api_key VARCHAR(512),
                model VARCHAR(128),
                timeout_seconds INT NOT NULL DEFAULT 1200,
                is_default TINYINT NOT NULL DEFAULT 0,
                enabled TINYINT NOT NULL DEFAULT 1,
                tool_choice_preference VARCHAR(16),
                created_by VARCHAR(128),
